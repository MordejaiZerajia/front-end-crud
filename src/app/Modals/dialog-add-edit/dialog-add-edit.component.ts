...
export class DialogAddEditComponent implements OnInit {
  ...
  constructor(
    ...
    // specify all fields in the form
  ...
  addEditEmployee() {
    const model: Employee = {
      ...
      hireDate: moment(this.formEmployee.value.hireDate, 'DD/MM/YYYY').format('DD/MM/YYYY')
    };
    ...
    if (this.dataEmployee == null) {
      ...
      this._employeeService.update(this.dataEmployee.idEmployee, model).subscribe({
        ...
        error: (e) => {
          this.displayAlert("Employee update failed", "Error");
        }
      });
    }
  }
  ...
  ngOnInit(): void {
    if (this.dataEmployee) {
      const formattedHireDate = moment(this.dataEmployee.hireDate, 'DD/MM/YYYY').toDate();
